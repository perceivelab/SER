...
from backbone.MobileNetV2 import InvertedResidual
...
class UNISAL(BaseModel):
    ...
    def __init__(self,
        ...
        self.skip_with_relu = skip_with_relu
        self.verbose = verbose

        ### Initialize backbone CNN
        # we remove classifier and linear layers because they are unused
        if hasattr(backbone, 'linear'):
            backbone.linear = torch.nn.Identity()
        ...
        self.skip_2x = self.make_skip_connection(self.cnn.feat_2x_channels, channels_2x, 2, self.drop_probs[1])
        ...
        self.skip_4x = self.make_skip_connection(self.cnn.feat_4x_channels, channels_4x, 2, self.drop_probs[2])
    ...
    def _make_gaussian_maps(x, gaussians, size=None, scaling=6.):
        ...
        meshgrids = torch.meshgrid(
            [torch.linspace(0, 1, size[0], dtype=dtype, device=device),
             torch.linspace(0, 1, size[1], dtype=dtype, device=device),], indexing='ij')
    ...
    def forward(self, x, target_size=(480,640), h0=None, return_hidden=False,
                source='Imagenet100', static=True):
        """
        Forward pass.

        Arguments:
            x: Input batch of dimensions [batch, channel, h, w].
        ...
        if static is None:
            static = x.shape[1] == 1 or self.sources == ('Imagenet100',)

        
        #Encoder 
        #im_feat_1x, im_feat_2x, im_feat_4x = self.cnn(x, 'saliency')
        skip_feature = self.cnn(x, 'saliency')
        _, _, im_feat_4x, im_feat_2x, im_feat_1x = skip_feature
        if self.skip_with_relu and self.cnn.return_prerelu:
            im_feat_4x = F.relu(im_feat_4x)
            im_feat_2x = F.relu(im_feat_2x)
            im_feat_1x = F.relu(im_feat_1x)
        ...
        im_feat = self.__getattr__('adaptation' + (source_str if self.ds_adaptation else ''))(im_feat)

        im_feat = F.interpolate(im_feat, size=x.shape[-2:], mode='nearest')

        outputs  = torch.sigmoid(im_feat)
        return outputs, skip_feature
